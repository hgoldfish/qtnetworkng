...
#ifndef QT_NO_DEBUG
            if (NLMSG_OK(hdr, quint32(len)))
                qWarning("NetworkInterface/AF_NETLINK: received unknown packet type (%d) or too short (%u)",
                         hdr->nlmsg_type, hdr->nlmsg_len);
            else
                qWarning("NetworkInterface/AF_NETLINK: received invalid packet with size %d", int(len));
#endif
            return;
        }
    }
...
static QList<NetworkInterfacePrivate *> getInterfaces(int sock, char *buf)
{
    QList<NetworkInterfacePrivate *> result;
    struct ifreq req;
    ...
    processNetlinkRequest(sock, &ifi_req.req, buf, BufferSize, [&](ifinfomsg *ifi, size_t len) {
        ...
        if (Q_UNLIKELY(iface->name.isEmpty())) {
            qWarning("NetworkInterface: found interface %d with no name", iface->index);
            delete iface;
        } else {
...
static void getAddresses(int sock, char *buf, QList<NetworkInterfacePrivate *> &result)
{
    ...
    processNetlinkRequest(sock, &ifa_req.req, buf, BufferSize, [&](ifaddrmsg *ifa, size_t len) {
        ...
        for (NetworkInterfacePrivate *candidate : qAsConst(result)) {
        ...
        if (Q_UNLIKELY(!iface)) {
            qWarning("NetworkInterface/AF_NETLINK: found unknown interface with index %d", ifa->ifa_index);
            return;
        }
        ...
        std::function<HostAddress(uchar *, int)> makeAddress = [=](uchar *ptr, int len) {
            HostAddress addr;
        ...
        for ( ; RTA_OK(rta, len); rta = RTA_NEXT(rta, len)) {
            int payloadLen = RTA_PAYLOAD(rta);
            uchar *payloadPtr = reinterpret_cast<uchar *>(RTA_DATA(rta));
            ...
            case IFA_CACHEINFO:
                if (size_t(payloadLen) >= sizeof(ifa_cacheinfo)) {
                    ...
                    std::function<QDeadlineTimer(quint32)> toDeadline = [](quint32 lifetime) -> QDeadlineTimer {
