...
public:
    ...
    bool handleIncomingPacket(quint32 channelNumber, const QByteArray &payload);
...
bool DataChannelPrivate::handleIncomingPacket(quint32 channelNumber, const QByteArray &payload)
{
    ...
    if (channelNumber == DataChannelNumber) {
        ...
        if (channel.isNull()) {
            ...
            if (!c->d_func()->handleIncomingPacket(payload)) {
                c->abort();
            }
        }
    } else {
...
#ifdef DEBUG_PROTOCOL
            qtng_debug << "coroutine is killed while sending packet.";
#endif
            setError(DataChannel::ProgrammingError);
            return abort();
        } catch(...) {
...
#ifdef DEBUG_PROTOCOL
                qtng_debug << "data channel is disconnected:" << header.size();
#endif
...
#ifdef DEBUG_PROTOCOL
                qtng_debug << QString::fromLatin1("packetSize %1 is larger than %2").arg(packetSize).arg(maxPacketSize);
#endif
...
bool VirtualChannelPrivate::handleIncomingPacket(const QByteArray &packet)
{
    ...
    return DataChannelPrivate::handleIncomingPacket(channelNumber, payload);
}
