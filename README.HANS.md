...
QtNetworkNg, 简称 qtng 是一个使用协程进行网络编程的工具库。以往的 C++ 网络编程库，一般都使用回调式的编程，或者线程式的编程。前者虽然性能极强，但晦涩难懂，逻辑在大段代码间跳转极易出错。而后者则容易触碰同步崩溃并且难以应对现代的大并发网络连接。协程式的网络编程已经发展多年，结合两者的优点，既有较强的性能，又编程简单，获得大量的支持。在 Python, C#, nim 等语言社区里面，协程式的网络编程已经是这些社区的主流网络编程方式，而在C++ 领域长期没有得到广泛的应用。

QtNetworkNg 主要借鉴了 Python 社区的 gevent 与 requests 两个极受欢迎的网络编程库。将它们的理念带到 C++ 社区，设计并实现了一个人性化的 API. 与 boost::asio, QtNetwork, POCO, libcurl 这些网络编程库相比，QtNetworkNg 的 API 非常的直接，经常能够用三五行代码即可完成传统工具库大段的代码功能。
